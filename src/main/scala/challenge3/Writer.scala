...
case class Writer[W, A](log: W, value: A) {
  ...
  def flatMap[B](f: A => Writer[W, B])(implicit M: Monoid[W]): Writer[W, B] = {
    val next = f(value)
    Writer(M.append(log, next.log), next.value)
  }
}
...
object Writer {
  ...
  def sequence[W: Monoid, A](writers: List[Writer[W, A]]): Writer[W, List[A]] =
    writers.foldRight(Writer.value[W, List[A]](Nil))((el, acc) =>
      for { x <- el; xs <- acc } yield x :: xs)
...
object Example {
  ...
  def stocks(data: List[Stock]): (Stats, List[Stock]) =
    Writer.sequence(data.map(stock =>
      Writer(
        Stats(stock.cents, stock.cents, stock.cents, 1)
      ,  Stock(stock.ticker, stock.date, stock.cents + (if (stock.cents > 10000) 10 else 1000))
      )
    )).run
